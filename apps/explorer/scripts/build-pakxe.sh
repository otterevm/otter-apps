...
# Build Pakxe Explorer multi-arch images
# Usage: ./build-pakxe.sh [amd64-server] [arm64-server]
...
BUILD_BRANCH="${BUILD_BRANCH:-pakxe}"

# SSH Configuration
SSH_KEY="${SSH_KEY:-$HOME/.ssh/id_ed25519-dokcer}"
SSH_USER="${SSH_USER:-root}"

# Pakxe Chain Configuration
VITE_CHAIN_NAME="Pakxe"
VITE_CHAIN_ID="7447"
VITE_RPC_URL="https://pakxe.otterevm.com/rpc"
...
echo -e "${GREEN}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║   Pakxe Explorer Multi-Arch Build                      ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
...
remote_build() {
    ...
    local remote_script="/tmp/build-pakxe-${arch}.sh"
...
echo "[\$ARCH] Building with Pakxe config..."
...
echo ""
log_success "Pakxe Explorer published!"
echo ""
echo "Tags:"
echo "  - $IMAGE_NAME:$TAG"
