...
function QRCodeModal({
	...
	}, [address, selectedToken, playBeep])
...
function BalanceDisplay({ address, token }: { address: string; token: Token }) {
	...
	return (
		<div
			style={{ padding: '10px', background: '#f5f5f5', borderRadius: '8px' }}
		>
...
function QuickScanModal({
	...
	onClose: () => void
	onScan: (recipient: string, token?: Token, amount?: string) => void
}) {
	return (
		<div
			style={{
...
export function Account() {
	...
	const [sendInitialState, setSendInitialState] = React.useState<{
		recipient?: string
		token?: Token
		amount?: string
	} | null>(null)
